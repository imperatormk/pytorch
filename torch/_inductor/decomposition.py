...
decomps_to_exclude: list[torch._ops.OpOverload | torch._ops.OpOverloadPacket] = [
    ...
    # Same reason as the CPU op above: the MPS flash pair must survive AOT so
    # inductor can lower it, otherwise the graph holds bmm/softmax and the
    # fused kernel only ever runs in eager.
    aten._scaled_dot_product_attention_flash_mps.default,
